...
const paidProviderPattern = /\b(?:OpenAI|Anthropic|Resend|Firecrawl|Stripe|generateText|generateObject|streamText|chat\.completions|responses\.create|messages\.create|emails\.send)\b/i;
...
export const importAwareSurfaceChecks: CheckDefinition[] = [
  importedAwarePaidEndpointCheck,
  importedAwareWebhookCheck,
  importedAwareVercelCronAuthCheck
];

export const replacedSurfaceCheckIds = new Set(importAwareSurfaceChecks.map((check) => check.id));
