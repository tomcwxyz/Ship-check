...
- [x] Framework-neutral Cloud R0 Web composition: bearer-vs-session credential selection, fail-closed bearer precedence, session-only token management, browser mutation Origin checks and an injected pre-auth rate-limit boundary.
- [x] Server-only Postgres Cloud runtime package over the existing store/service/Web contracts, with transaction ownership, bounded pool configuration and database-URL redaction before application error hooks.
- [x] Thin Next.js 15 Node route binding over the landed Cloud runtime, with health endpoint, pilot-only browser session bootstrap and pre-auth in-process rate limit for first deployment.
- [ ] Replace the pilot session bootstrap with normal hosted session verification and replace the in-process limiter with an operational shared rate-limit store before broad access.
- [ ] Create/link the Vercel project, configure server-only Neon/pilot environment values and verify a deployed preview before production promotion.
...
- [ ] Bind the landed HTTP rename/retention/export/deletion routes to real hosted authentication/product flows and verify them end-to-end before private-project pilot use.
